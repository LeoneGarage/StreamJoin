...
j = (
  ...
  .agg(F.max("total_amount").alias("max_amount"))
  .reduce(column = "max_amount", update = F.greatest(F.col("u.max_amount"), F.col("staged_updates.max_amount")), delta_update = F.greatest(F.col("p.max_amount"), F.col("m.max_amount")))
...
jj = cc_tt_g_tt.groupBy("transaction_date").agg(F.max("total_amount").alias("max_amount"))
jj.count()
